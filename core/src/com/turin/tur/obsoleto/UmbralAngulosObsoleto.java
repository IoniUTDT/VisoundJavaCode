...
public class UmbralAngulosObsoleto {
	...
	public static class Info {
		...
		public static class LevelAdvance {
			public Array<DinamicaExperimento> convergencias = new Array<DinamicaExperimento>();
		}
	...
	public DinamicaExperimento cuadranteActivo; // En cual de los cuadrantes esta la señal que se va a medir
	...
	public void levelCompleted() {
		...
		for (DinamicaExperimento cuadrante : this.info.advance.convergencias) {
	...
	public void makeLevels() {
		...
		this.info = json.fromJson(UmbralAngulosObsoleto.Info.class, savedData);
		...
		while (angulosdeReferencia.size>0) { // Seleccionamos angulos en forma random segun parametros del setup
			...
			for (int anguloReferencia:angulosReferenciaElegidos) {
				...
				DinamicaExperimento cuadrante1 = new DinamicaExperimento();
				DinamicaExperimento cuadrante2 = new DinamicaExperimento();
				DinamicaExperimento cuadrante3 = new DinamicaExperimento();
				DinamicaExperimento cuadrante4 = new DinamicaExperimento();
				...
				for (Entry<Integer, Integer> recurso:this.info.indexs.idsResourcesBySides.get(anguloReferencia).entries()) {
					...
					if (this.cumpleCriterioDistanciaMinima(anguloReferencia, recurso.key)) {
						...
						JsonTrial trial = Builder.crearTrial("Selecciones a que categoria pertenece el angulo", "", DISTRIBUCIONESenPANTALLA.LINEALx3,
								new int[] {Constants.Resources.CategoriasImagenes.Grave.ID,Constants.Resources.CategoriasImagenes.Recto.ID,Constants.Resources.CategoriasImagenes.Agudo.ID}, TIPOdeTRIAL.TEST, recurso.value, false, true, false);
		...
		for (int indice1 = 0; indice1<this.info.setup.angulos.size ; indice1++) {
			for (int indice2 = indice1 +1; indice2<this.info.setup.angulos.size; indice2++) {
				...
				if (this.cumpleCriterioDistanciaMinima(angulo1, angulo2)) {
					...
					// imagen.idVinculo = "";
					imagen.categories.add(CategoriasImagenes.Angulo);
